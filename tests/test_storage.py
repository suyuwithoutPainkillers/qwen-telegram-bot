...
class StorageTest(unittest.TestCase):
    ...
    def test_model_history_trim_and_clear(self) -> None:
        ...
        self.assertEqual(history[-1]["content"], "model 24")
...
class UtilsSqliteTest(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self) -> None:
        ...
        utils.available_models = ["qwen-turbo", "qwen-plus", "qwen-max"]
        access_control.init_admin_user_ids("100")

    async def asyncTearDown(self) -> None:
        utils.chat_dict.clear()
        utils.client = None
        utils.available_models = list(utils.DEFAULT_QWEN_MODELS)
    ...
    async def test_restore_switch_clear_and_image_summary(self) -> None:
        ...
        self.assertEqual(session["model"], "qwen-turbo")

        await utils.select_model(7, "qwen-turbo")
        await utils.save_turn(7, "hello", "hi")
        await utils.save_turn(7, [Image.new("RGB", (1, 1)), "what is this?"], "an image answer")
        ...
        self.assertEqual(restored["model"], "qwen-turbo")

        await utils.select_model(7, "qwen-plus")
        switched = utils.chat_dict[7]
        self.assertEqual(switched["model"], "qwen-plus")

        await utils.clear_history(7)
        self.assertEqual(storage.get_user_model(7), "qwen-plus")
